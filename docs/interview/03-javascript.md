...
### 1、原型的作用
>[!tip] 原型
> - 用来 **实现面向对象，标注对象的类型**，原型的存在避免了类型丢失。
> - 每一个函数都由一个属性：`prototype`,它的值是一个对象。
> - `prototype`属性中有一个属性：`constructor`，它的值是原本的这个函数。
...
### 2、原型链的理解
>[!tip] 原型链
> - 所有对象都有原型，原型是 对象所属的原型对象，所有对象都可以访问原型对象的属性和方法。
...
>[!tip] 单线程的必然性
>1. 避免DOM操作的冲突：
>   - 浏览器的渲染线程和事件处理线程是互斥的，这就保证了DOM操作的安全。
>   - 单线程通过事件循环 顺序执行 JavaScript 和 DOM 操作，确保操作的原子性。
>   - 单线程避免了多线程同步渲染和逻辑的复杂性。
>2. 简化开发复杂度：
>   - 多线程编程需要处理锁、同步等复杂问题。
>   - 单线程没有线程切换的开销，无需关心线程安全问题。
>   - 单线程+异步非阻塞模型，足以处理 I/O 密集型操作。。
