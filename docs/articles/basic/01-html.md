...
tags:
  - HTML
excerpt: "超文本标记语言 是构建网页的标准语言。 用于定义网页的结构和内容，通过一系列标签来标识文本、图像、链接、表单等网页元素。 特点： 跨平台： 兼容性强：HTML可以在多种操作系统和浏览器上运行。 设备无关性：HTML网页可以在不同的设备上显示..."
---
...
>[!TIP] 常见的HTML标签：
> - 标题标签：`<h1>`~`<h6>`
> - 文字标签：`<p>`、`<span>`、`<a>`、`<em>`、`<strong>`、`<b>`、`<i>`、`<u>`
> - 多媒体标签：`<img>`、`<audio>`、`<video>`、`<svg>`、`<iframe>`、`<object>`
> - 列表标签：`<ul>`、`<ol>`、`<li>`
> - 表单标签：`<form>`、`<input>`、`<option>`、`<textarea>`、`<button>`
> - 表格标签：`<table>`、`<tr>`、`<td>`、`<th>`
> - 其他标签：`<div>`、`<canvas>`、`<header>`、`<footer>`、`<nav>`、`<main>`、`<article>`、`<section>`、`<aside>`、`<details>`、`<figure>`、`<figcaption>`、`<dialog>`、`<datalist>`、`<output>`、`<math>`、`<picture>`、`<mark>`
...
#### 2.4、列表标签
>[!TIP]
> 列表标签用于定义网页的列表内容，如有序列表、无序列表、自定义列表等。
> - 有序列表`<ol>`、无序列表`<ul>`。
> - 自定义列表`<dl>`、自定义列表项`<dt>`、自定义列表描述`<dd>`。

##### 2.4.1、有序列表
>[!TIP]
> 有序列表用于定义有顺序的列表项，使用`<ol>`标签。
> - 列表项`<li>`。
> - 列表标记`<li>`的`type`属性，可选值：`1`、`a`、`A`、`i`、`I`。
> - 列表标记`<li>`的`start`属性，设置列表项的起始值。
...
##### 2.4.2、无序列表
>[!TIP]
> 无序列表用于定义无顺序的列表项，使用`<ul>`标签。
> - 列表项`<li>`。
> - 列表标记`<li>`的`type`属性，可选值：`circle`、`disc`、`square`。
...
##### 2.4.3、自定义列表
>[!TIP]
> 自定义列表用于定义自定义的列表项，使用`<dl>`标签。
> - 自定义列表项`<dt>`、自定义列表描述`<dd>`。
