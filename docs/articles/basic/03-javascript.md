...
| find(callback)                 | 返回第一个满足条件的元素的值。若没有返回`undefined` |
| findIndex(callback)            | 返回第一个满足条件的元素的索引，若没有返回 `-1`|
...
| slice(startIndex,endIndex)   | 切片，包含`startIndex`，不含`endIndex`，返回新数组       |
| join(Arr)    | 将数组转为字符串，返回字符串   |
| flat()    | 扁平化数组，返回新数组     |
| flatMap() | 映射后扁平化，返回新数组   |
| indexOf(item)     | 返回数组中第一次出现给定元素的下标，如果不存在则返回 `-1` |
| lastIndexOf(item) | 返回数组给定元素最后一次出现的下标，如果不存在则返回 `-1` |
...
| 方法名称                  | 描述              |
|-----------------------|-----------------|
| Array.from(arrayLike) | 从类数组或可迭代对象，创建新数组 |
